...
*/
var exportProductToAmazon = function (args) {
    ...
    while (products.hasNext()) {
        ...
        if (amazonProducts.indexOf(product.ID) == -1 && product.custom.amazonProductExport && product.online) {
            ...
            Logger.info("loaResponse amazon product id before" + JSON.stringify(loaResponse));
            amazonProducts.push(product.ID);
            ...
            Logger.info("amazon product id before" + product.ID);
            if (product.custom.amazonProduct_Type == null) {
                Transaction.wrap(function () {
                    product.custom.amazonProduct_Type = !empty(loaResponse) ? loaResponse.results.category : '';
                });
            }
            if (product.ID && product.brand && product.UPC && condition && standardPrice && color_name && product.custom.country_of_origin && material_composition && product.shortDescription && product.shortDescription.source && category && subCategory) {
                if (subCategory == 'Earrings' && product.custom.amazonEarring_Design && (product.custom.amazonEarring_Design.value == null || empty(product.custom.amazonEarring_Design.value))) {
                    skipProductList.push({
                        ...
                        earring_design: product.custom.amazonEarring_Design.value ? product.custom.amazonEarring_Design.value : '',
                ...
                skipProductList.push({
                    ...
                    fabric_type: product.custom.lining ? product.custom.lining : '',
                    item_weight: product.custom.weight ? product.custom.weight : '',
                    earring_design: product.custom.amazonEarring_Design ? product.custom.amazonEarring_Design : '',
                    amazon_today: (product.custom.amazonTodayProductExport !== null && !empty(product.custom.amazonTodayProductExport) && product.custom.amazonTodayProductExport) ? product.custom.amazonTodayProductExport : false,
                    image: images
                });
            }

            Logger.info("amazon product id after" + product.ID);
        ...
        Logger.info("amazon product =" + amazonProducts.length);
        Logger.info("skipped product length =" + skipProductList.length);
        //Logger.info("skipped product list" + JSON.stringify(skipProductList));
        try {
            var targetFolder = new File(File.IMPEX + File.SEPARATOR + 'amazon-feed-products');
            var siteId = Site.getCurrent().ID;
            if (!targetFolder.exists()) {
                targetFolder.mkdirs();
            }

            var newFile = new File(targetFolder.fullPath + File.SEPARATOR + siteId + '-amazon-products.json');
            ...
            var data = JSON.stringify(amazonProductsList);
            fileWriter.writeLine(data);
            fileWriter.close();
        } catch (e) {
            Logger.error("amazon failed " + e);
...
function generateCsvForSkippedProducts(skippedProductList) {
    try {
        ...
        var siteId = Site.getCurrent().ID;
        var csvFile = new File(targetFolder.fullPath + File.SEPARATOR + siteId + '-skipped-amazon-products.csv');
        if (!csvFile.exists()) {
...
exports.exportProductToAmazon = exportProductToAmazon;
