...
function generateCSVHeader(exportType) {
    ...
    if (exportType === FConstants.EXPORT_TYPE.CATALOG) {
        ...
        csvHeaderArray.push(FConstants.HEADER_VALUES.SIZE);
        ...
        csvHeaderArray.push(FConstants.HEADER_VALUES.GENDER);
        csvHeaderArray.push(FConstants.HEADER_VALUES.PRIMARY_CATEGORY);
        csvHeaderArray.push(FConstants.HEADER_VALUES.MODEL);
        csvHeaderArray.push(FConstants.HEADER_VALUES.LAUNCHDATE);
        csvHeaderArray.push(FConstants.HEADER_VALUES.MATERIAL);
        csvHeaderArray.push(FConstants.HEADER_VALUES.COLOR);
        csvHeaderArray.push(FConstants.HEADER_VALUES.CLOSURE);
        csvHeaderArray.push(FConstants.HEADER_VALUES.LOCATION);
        csvHeaderArray.push(FConstants.HEADER_VALUES.CONDITION);
        csvHeaderArray.push(FConstants.HEADER_VALUES.CATEGORY);
        csvHeaderArray.push(FConstants.HEADER_VALUES.SALE_PRICE);
        csvHeaderArray.push(FConstants.HEADER_VALUES.SEARCHABLE);
        csvHeaderArray.push(FConstants.HEADER_VALUES.SEARCHABLE_IF_UNAVAILABLE);
...
function getCondition(product) {
    ...
    var markup = headlineDescription && typeof headlineDescription.markup === 'string'
        ? headlineDescription.markup.toLowerCase()
        : '';
    if (!markup) {
        return '';
    }
    if (markup.includes('flawed gem condition')) {
        return 'flawed gem';
    } else if (markup.includes('pristine condition')) {
        return 'pristine';
    } else if (markup.includes('excellent condition')) {
        return 'excellent';
    } else if (markup.includes('very good condition')) {
        return 'very good';
    } else if (markup.includes('good condition')) {
        return 'good';
    }
    return '';
 }
