...
GiftCardStrategy.prototype.validatePayload = function (actionType) {
    ...
    try {
        ...
        this.logger.error('ALTERNATIVE_PAYMENTS_ERROR. GIFT_CARD. VALIDATE_PAYLOAD : {0}', this.logger.message(e));
        throw e;
    }
};
...
GiftCardStrategy.prototype.setDefaultErrorResponse = function (errorMsg, actionType) {
    try {
        ...
        this.logger.error('ALTERNATIVE_PAYMENTS_ERROR. GIFT_CARD. SET_DEFAULT_RESPONSE : {0}', this.logger.message(e));
    }
};

/**
 * Processes creation of the new Gift Card
 * @param {Object} order - SFCC order
 */
GiftCardStrategy.prototype.create = function (order) {
    var globaleHelpers = require('*/cartridge/scripts/helpers/globaleHelpers');
    ...
    try {
        ...
        var giftCardProvider = (require('*/cartridge/scripts/globale/alternativePayments/providers/index'))['giftCard']; // eslint-disable-line dot-notation
        if (!giftCardProvider) {
            throw Error('Unsupported gift card provider: ' + giftCardProvider + '. Order ID: ' + order.getOrderNo());
        }
        collections.forEach(order.getAllProductLineItems(), function (pli) {
            if (pli.custom[globaleHelpers.customAttr.productLineItem.geIsGiftCard] === true) {
                for (var i = 0; i < pli.quantityValue; i++) {
                    giftCardProvider.create(order, pli);
                }
            }
        });
    } catch (e) {
