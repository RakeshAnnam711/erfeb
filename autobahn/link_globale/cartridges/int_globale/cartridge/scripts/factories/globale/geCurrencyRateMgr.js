...
function getGlobaleObject(key) {
    ...
    try {
        var provider = geObjectDataProvider.createDataProvider(
            require('*/cartridge/scripts/globale/cache/currencyRateCacheMgr'),
            globaleHelpers.customObjectKeys.coCurrencyRates
        );
        result = provider.getGEObject(key);
    } catch (e) {
        Logger.getLogger('GLOBALE').error('geCurrencyRateMgr: was not able to get Global-E object. Error: ' + e.message);
        throw e;
    }
...
module.exports = {
    getGECurrencyRate: getGlobaleObject
};
