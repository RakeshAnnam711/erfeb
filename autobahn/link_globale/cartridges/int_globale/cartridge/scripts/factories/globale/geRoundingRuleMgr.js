...
function getGlobaleObject(key) {
    ...
    try {
        var provider = geObjectDataProvider.createDataProvider(
            require('*/cartridge/scripts/globale/cache/roundingRulesCacheMgr'),
            globaleHelpers.customObjectKeys.coRoundingRules
        );
        result = provider.getGEObject(key);
    } catch (e) {
        Logger.getLogger('GLOBALE').error('geRoundingRuleMgr: was not able to get Global-E object. Error: ' + e.message);
        throw e;
    }
...
module.exports = {
    getGERoundingRule: getGlobaleObject
};
