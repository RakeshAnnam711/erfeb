...
WeightField.prototype.getValue = function () {
    ...
    if (this.product && this.column.attr && this.column.attr.length > 0) {
        ...
        if (result && this.column.unit && !isNaN(result) && !isNaN(parseFloat(result))) { // eslint-disable-line no-restricted-globals
            switch (this.column.unit) {
                case 'kg':
                    result = result * 1000; // eslint-disable-line operator-assignment
                    break;
                default:
                    break;
            }
 ...
 */
WeightField.prototype.isValidColumn = function () {
    var valid = true;
