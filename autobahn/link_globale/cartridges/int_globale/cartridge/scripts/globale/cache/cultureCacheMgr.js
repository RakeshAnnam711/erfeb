...
function getCacheObjectFromCO(co) {
    return {
        code: co.custom.code,
        regionCode: co.custom.regionCode,
        cityCode: co.custom.cityCode,
        culture: co.custom.culture,
        name: co.custom.name,
        localizedName: co.custom.localizedName,
        isRTL: co.custom.isRTL
    };
}
...
function getCachedValue(key) {
    ...
    var cache = require('dw/system/CacheMgr').getCache(globaleHelpers.cacheKeys.cultures);

    return cache.get(require('dw/system/Site').current.ID + '_' + key, function () {
        var co = CustomObjectMgr.getCustomObject(globaleHelpers.customObjectKeys.coCultures, key);
...
function isCacheEnabled() {
    ...
    return cacheHelpers.isEnabled(globaleHelpers.cacheKeys.cultures);
}
