...
function getCacheObjectFromCO(co) {
    return {
        roundingRuleId: co.custom.roundingRuleId,
        currencyCode: co.custom.currencyCode,
        countryCode: co.custom.countryCode,
        roundingRanges: co.custom.roundingRanges
    };
}
...
function getCachedValue(key) {
    ...
    var cache = require('dw/system/CacheMgr').getCache(globaleHelpers.cacheKeys.roundingRules);

    return cache.get(require('dw/system/Site').current.ID + '_' + key, function () {
        var co = CustomObjectMgr.getCustomObject(globaleHelpers.customObjectKeys.coRoundingRules, key);
...
function isCacheEnabled() {
    ...
    return cacheHelpers.isEnabled(globaleHelpers.cacheKeys.roundingRules);
}
