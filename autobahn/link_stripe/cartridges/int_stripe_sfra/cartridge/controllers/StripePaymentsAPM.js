...
server.get('GetPaymentElementOptions', function (req, res, next) {
    ...
    if (customer.authenticated && customer.profile && customer.profile.email) {
        ...
        // Do not force setup_future_usage on Payment Element options because
        // it can hide one-time payment methods (for example, Affirm).
...
server.post('StripeQuickCheckout', csrfProtection.validateAjaxRequest, function (req, res, next) {
    ...
    var shippingName = (req.form.shippingName || '').trim().split(/\s+/);
    var shippingFirstName = shippingName[0];
    var shippingLastname = shippingName.slice(1).join(' ');
    ...
    var billingName = (req.form.billingName || '').trim().split(/\s+/);
    var billingFirstName = billingName[0];
    var billingLastName = billingName.slice(1).join(' ');
