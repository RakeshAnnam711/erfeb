...
//     enabled: true
// },
return_url: require('dw/web/URLUtils').https('Order-Confirm', 'ID', paymentInstrument.paymentTransaction.custom.stripeOrderNumber).toString()
    };

    if (paymentInstrument.custom.stripePRUsed) {
        createPaymentIntentPayload.automatic_payment_methods = {
            enabled: true
        };

        createPaymentIntentPayload.excluded_payment_method_types = [
            'amazon_pay',
            'google_pay',
            'link',
            'paypal'
...
exports.getShippingOptionsSFRA = function (params) {
    ...
    function isMethodAllowed(shippingMethod, shippingCountryCode) {
        if (shippingMethod.custom.storePickupEnabled) {
            return false;
        }

        if (shippingCountryCode === 'US') {
            var methodId = (shippingMethod.ID || '').toUpperCase();
            var methodName = (shippingMethod.displayName || '').toUpperCase();
            var methodDescription = (shippingMethod.description || '').toUpperCase();
            var isGlobale = methodId.indexOf('GLOBALE') !== -1
                || methodName.indexOf('GLOBAL-E') !== -1
                || methodName.indexOf('GLOBALE') !== -1
                || methodDescription.indexOf('GLOBAL-E') !== -1
                || methodDescription.indexOf('GLOBALE') !== -1;
    ...
    if (params.shippingAddress) {
        ...
        Transaction.wrap(function () {
            ...
            shippingAddress.setAddress2(stripeShippingAddress.line2 || stripeShippingAddress.line1);
    ...
    if (params.selectedShippingRateId) {
        ...
        var shipmentAddress = shipments[0] && shipments[0].shippingAddress;
        var shippingCountryCode = shipmentAddress && shipmentAddress.countryCode ? shipmentAddress.countryCode.value : null;

        if (!shippingCountryCode && request && request.locale && request.locale.indexOf('_US') !== -1) {
            shippingCountryCode = 'US';
        }

        for (let i = 0; i < shippingMethods.length; i++) {
            var shippingMethod = shippingMethods[i];
            if (isMethodAllowed(shippingMethod, shippingCountryCode)) {
...
exports.getShippingOptionsSiteGenesis = function (params) {
    ...
    var shippingAddressParam = !empty(params.shippingAddress) && !empty(params.shippingAddress.stringValue)
        ? params.shippingAddress.stringValue
        : params.shippingAddress;
    var shippingNameParam = !empty(params.shippingName) && !empty(params.shippingName.stringValue)
        ? params.shippingName.stringValue
        : params.shippingName;
    var phoneParam = !empty(params.phone) && !empty(params.phone.stringValue)
        ? params.phone.stringValue
        : params.phone;

    if (!empty(shippingAddressParam) && !empty(shippingNameParam) && currentBasket && currentBasket.defaultShipment) {
        var stripeShippingAddress = JSON.parse(shippingAddressParam);
        var shippingNameParts = String(shippingNameParam).split(' ');
        var shippingFirstName = shippingNameParts[0] || '';
        var shippingLastName = shippingNameParts.slice(1).join(' ') || '-';
        var shipment = currentBasket.defaultShipment;
        ...
        Transaction.wrap(function () {
            ...
            if (!empty(phoneParam)) {
                shippingAddress.setPhone(phoneParam);
            }
        });
    }
