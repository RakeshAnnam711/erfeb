...
function sendConfirmationEmailSafely(order) {
    if (!order) {
        return false;
    }
    ...
    try {
        if (isSFRA()) {
            ...
            COHelpers.sendConfirmationEmail(order, customerLocaleID ? customerLocaleID : 'default');
            return true;
        }

        var statusMail = sendMail(order, 'mail/orderconfirmation', Resource.msg('order.orderconfirmation-email.001', 'order', null));
        return statusMail && statusMail.status === Status.OK;
    } catch (e) {
        logger.error('Order {0}: send confirmation email failed in Stripe webhook job. Error: {1}', order.orderNo, String(e));
        return false;
    }
}

function toMessageArray(message) {
    if (empty(message)) {
        return [];
    }
...
function runDoPrePlaceOrderSafely(order) {
    if (!order) {
        return;
    }
    ...
    try {
        ...
        if (doPrePlaceOrderResult.error) {
            var messages = toMessageArray(doPrePlaceOrderResult.message);

            if (messages.length) {
                Transaction.wrap(function () {
                    messages.forEach(function (message) {
                        if (!empty(message)) {
                            order.addNote('Failed order pre processing: ', String(message));
                        }
                    });
                });
            }
        ...
        logger.error('Order {0}: doPrePlaceOrder failed in Stripe webhook job. Error: {1}', order.orderNo, String(e));
        Transaction.wrap(function () {
            order.addNote('Failed order pre processing: ', String(e));
        });
    }
}
...
function createCharge(stripeNotificationObject, order, stripePaymentInstrument) {
    ...
    try {
        if (order.status.value === Order.ORDER_STATUS_CREATED) {
            runDoPrePlaceOrderSafely(order);
        }
...
function placeOrder(stripeNotificationObject, order, stripePaymentInstrument) { // eslint-disable-line
    ...
    if (isAPMOrder) {
        var emailSent = sendConfirmationEmailSafely(order);

        stripeLogger.info('Successfully proccesed CO with event id: {0}, source id: {1} , updated SFCC order status to "EXPORT_STATUS_READY". Set up CO processingStatus to {2}, email send - {3}', stripeNotificationObject.custom.stripeEventId, stripeNotificationObject.custom.stripeSourceId, 'PROCESSED', emailSent ? 'true' : 'false');
    } else {
...
function processNotificationObject(stripeNotificationObject) {
    ...
    var stripeEventType = stripeNotificationObject.custom.stripeType;
    ...
    if (stripeAPMPaymentInstrument && stripePaymentInstrument.custom.stripeSourceID) {
        ...
        var isLegacySourceEvent = stripeEventType && stripeEventType.indexOf('source.') === 0;
        var isLegacySourceIdPair = coStripeSourceId && piStripeSourceId
            && coStripeSourceId.indexOf('src_') === 0
            && piStripeSourceId.indexOf('src_') === 0;
        var enforceSourceMatch = isLegacySourceEvent || isLegacySourceIdPair;

        if (enforceSourceMatch && coStripeSourceId !== piStripeSourceId) {
            stripeLogger.info(
        ...
        if (!enforceSourceMatch && coStripeSourceId && coStripeSourceId !== piStripeSourceId) {
            stripeLogger.info(
                '\nSource IDs differ but continuing for non-legacy flow, SFCC order id: {0}, CO event id: {1}, webhook source: {2}, PI source: {3}, eventType: {4}',
                orderNo,
                stripeEventId,
                coStripeSourceId,
                piStripeSourceId,
    ...
    try {
        ...
        while (stripeObjectsIter.hasNext()) {
            ...
            try {
                ...
                logger.error(
                    'Failed processing Stripe webhook notification. eventId={0}, sourceId={1}, type={2}, error={3}',
                    stripeNotificationObject.custom.stripeEventId,
                    stripeNotificationObject.custom.stripeSourceId,
                    stripeNotificationObject.custom.stripeType,
                    String(err)
                );
            }
        }
