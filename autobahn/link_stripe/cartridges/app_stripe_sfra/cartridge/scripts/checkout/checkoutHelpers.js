...
if (stripeHelper.isStripeEnabled()) {
    ...
    COHelpers.buildPaymentIntentPayload = function buildPaymentIntentPayload(order, req, stripeChargeCapture, stripePaymentInstrument) {
        ...
        // Primary source of truth: non-gift certificate payment instrument amount.
        // This keeps Stripe aligned with split-payment calculation already applied on the basket/order.
        var payableAmount = stripePaymentInstrument
            && stripePaymentInstrument.paymentTransaction
            && stripePaymentInstrument.paymentTransaction.getAmount();

        // Fallback to order total minus gift certificates if payment transaction amount is unavailable.
        if (empty(payableAmount)) {
            ...
            payableAmount = orderTotal.subtract(giftCertTotal);
        }

        var amount = Math.round(payableAmount.getValue() * multiplier);
        ...
        if (customer.authenticated && customer.profile && customer.profile.email) {
            ...
            if (!empty(confirmationToken)) {
                var setupFutureUsage = confirmationToken.setup_future_usage;
                if (!empty(setupFutureUsage) && (setupFutureUsage === 'on_session' || setupFutureUsage === 'off_session')) {
                    createPaymentIntentPayload.setup_future_usage = setupFutureUsage;
                }
            }
        }
