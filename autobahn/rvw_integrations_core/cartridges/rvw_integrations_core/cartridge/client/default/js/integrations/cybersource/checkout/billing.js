        ...
        for (var paymentInstrument of order.billing.payment.selectedPaymentInstruments) {
            if (paymentInstrument.paymentMethod === "GIFT_CERTIFICATE") {
                ...
                if (isCSType) {
                    htmlToAppend +=
                        '<div class="mb-3">'
                        + '<span>Secure Acceptance ' + paymentInstrument.paymentMethod.replace('_', ' ') + '</span>'
                        + '</div>'
                } else if(paymentInstrument.paymentMethod === 'PayPal') { // PayPal Standalone
                    hasPaypal = true;
                    htmlToAppend +=
                        '<div class="mb-3">'
                        + '<span>' + paymentInstrument.paymentMethod + '</span>'
                        + '<div><span>' + paymentInstrument.formattedAmount + '</span></div>'
                        + '</div>'
                } else if(paymentInstrument.paymentMethod === 'PAYPAL' || paymentInstrument.paymentMethod === 'PAYPAL_CREDIT') {
                    hasPaypal = true;
                    htmlToAppend +=
                        '<div class="mb-3">'
                        + '<span>' + paymentInstrument.paymentMethod + '</span>'
                        + '<div><span>' + paymentInstrument.amount + '</span></div>'
                        + '</div>'
                } else if (paymentInstrument.paymentMethod === 'CREDIT_CARD') {
                    htmlToAppend +=
                        '<div class="mb-3">'
                        ...
                        + '<div><span>' + order.resources.cardEnding + ' ' + paymentInstrument.expirationMonth + '/' + paymentInstrument.expirationYear + '</span></div>'
                        + '</div>'
                } else {
                    $('.paypalDetails').addClass('show');
                    htmlToAppend +=
                        '<div class="mb-3">'
                        + paymentInstrument.paymentMethod + '</span>'
                        + '</div>'
                }
            }
        }
