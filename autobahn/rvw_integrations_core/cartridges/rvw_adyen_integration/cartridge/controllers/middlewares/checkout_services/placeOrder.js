...
function placeOrder(req, res, next) {
  ...
  var preAuthFraudCheckResult = checkoutServicesHelpers.placeOrderPreAuthFraudCheck(order, currentBasket);

  // WGACA MODIFICATION - Additional Logging
  dw.system.Logger.warn('rvw_integrations_core\\cartridges\\rvw_adyen_integration\\cartridge\\controllers\\middlewares\\checkout_services\\placeOrder.js: preAuthFraudCheckResult for the customer {0} : {1}', currentBasket.customerEmail, preAuthFraudCheckResult.error);
  ...
  if (preAuthFraudCheckResult.error) {
      ...
      // WGACA MODIFICATION - Additional Logging
      dw.system.Logger.warn('rvw_integrations_core\\cartridges\\rvw_adyen_integration\\cartridge\\controllers\\middlewares\\checkout_services\\placeOrder.js: preAuthFraudCheckResult failed for the customer {0} :', currentBasket.customerEmail);
  ...
  if (order.getCustomerEmail()) {
    //AUTOBAHN MODIFICATION try/catch for safety
    try {
