...
function submitPayment(req, res, next) {
    ...
    if (!paymentProcessor) {
        var paymentMethod = PaymentMgr.getPaymentMethod(paymentMethodIdValue);
        if (paymentMethod && paymentMethod.paymentProcessor) {
            paymentProcessor = paymentMethod.paymentProcessor.ID.toLowerCase();
        }
    }
    ...
    this.on('route:BeforeComplete', function (req, res) { // eslint-disable-line no-shadow
        ...
        if (!paymentProcessor) {
            ...
            paymentProcessor = paymentProcessor.ID.toLowerCase();
        }
...
function doForAllPaymentInstruments(lineItemContainer, callback) {
    ...
    for (var paymentInstrumentIndex in lineItemContainer.paymentInstruments) {
        var paymentProcessor = dw.order.PaymentMgr.getPaymentMethod(lineItemContainer.paymentInstruments[paymentInstrumentIndex].paymentMethod).paymentProcessor
        if (!paymentProcessor) {
            throw new Error(Resource.msg(
                'error.payment.processor.missing',
                'checkout',
