...
server.get('OrderSummary', function (req, res, next) {
    if (subproEnabled) {
        ...
        res.render('subpro/cart/subprooptions', { subproData: {
            product: product,
            productSchedule: productSchedule,
            page: 'order-summary'
        }});
...
server.get('OrderConfirmation', function (req, res, next) {
    if (subproEnabled) {
        var data = SubscribeProLib.getOrderConfirmationData(params.orderNumber.stringValue, params.sku.stringValue);
        if (data) {
            res.render('subpro/cart/subprooptions', { subproData: data});
        }
    }
    next();
});
...
server.post('UpdateOptions', function (req, res, next) {
    if (subproEnabled) {
        ...
        require('dw/system/Transaction').wrap(function () {
            ...
            if (params.subscriptionMode.toString() === 'regular' && discountValue != 0) {
