...
function appendQueryParams(target, queryParamsToAppend) {
    // AUTOBAHN MODIFICATION - Cannot initiate Querystring on URL String without querystring value '?...'
    // var resource = target.split('?')[0];
    // var targetQueryString = new QueryString(target);
    var targetSplit = target.split('?');
    var resource = targetSplit[0];
    var targetQueryString = new QueryString(targetSplit[1] || '');
