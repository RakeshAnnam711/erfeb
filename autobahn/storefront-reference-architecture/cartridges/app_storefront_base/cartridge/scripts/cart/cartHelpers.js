...
function addLineItem(
    ...
    if (product.bundle && childProducts.length) {
        module.exports.updateBundleProducts(productLineItem, childProducts); // AUTOBAHN MOD - Update for project specific extensions
    }
 ...
 */
function excludeUuid(selectedUuid, itemUuid) { // eslint-disable-line no-unused-vars
    return selectedUuid
    ...
    collections.forEach(lineItems, function (item) {
        if (item.bundledProductLineItems.length) {
            collections.forEach(item.bundledProductLineItems, function (bundleItem) {
                if (bundleItem.productID === productId && module.exports.excludeUuid(uuid, bundleItem.UUID)) { // AUTOBAHN MOD - Update for project specific extensions
                    qtyAlreadyInCart += bundleItem.quantityValue;
                }
            });
        } else if (item.productID === productId && module.exports.excludeUuid(uuid, item.UUID)) { // AUTOBAHN MOD - Update for project specific extensions
...
function getExistingProductLineItemsInCart(product, productId, productLineItems, childProducts, options) {
    var matchingProductsObj = module.exports.getMatchingProducts(productId, productLineItems); // AUTOBAHN MOD - Update for project specific extensions
    ...
    var productLineItemsInCart = matchingProducts.filter(function (matchingProduct) {
        // AUTOBAHN MOD - Update for project specific extensions
        return product.bundle
            ? module.exports.allBundleItemsSame(matchingProduct.bundledProductLineItems, childProducts)
            : module.exports.hasSameOptions(matchingProduct.optionProductLineItems, options || []);
    });
...
function getExistingProductLineItemInCart(product, productId, productLineItems, childProducts, options) {
    return module.exports.getExistingProductLineItemsInCart(product, productId, productLineItems, childProducts, options)[0]; // AUTOBAHN MOD - Update for project specific extensions
}
...
function checkBundledProductCanBeAdded(childProducts, productLineItems, quantity) {
    ...
    canBeAdded = childProducts.every(function (childProduct) {
        ...
        totalQtyRequested = itemQuantity + module.exports.getQtyAlreadyInCart(childPid, productLineItems); // AUTOBAHN MOD - Update for project specific extensions
...
function addProductToCart(currentBasket, productId, quantity, childProducts, options) {
    ...
    if (product.bundle) {
        canBeAdded = module.exports.checkBundledProductCanBeAdded(childProducts, productLineItems, quantity); // AUTOBAHN MOD - Update for project specific extensions
    } else {
        totalQtyRequested = quantity + module.exports.getQtyAlreadyInCart(productId, productLineItems); // AUTOBAHN MOD - Update for project specific extensions
    ...
    // AUTOBAHN MOD - Update for project specific extensions
    productInCart = module.exports.getExistingProductLineItemInCart(product, productId, productLineItems, childProducts, options);
    ...
    if (productInCart) {
        ...
        var productLineItem;
        // AUTOBAHN MOD - Update for project specific extensions
        productLineItem = module.exports.addLineItem(
            currentBasket,
            product,
            quantity,
            childProducts,
...
module.exports = {
    ...
    excludeUuid: excludeUuid, // AUTOBAHN MOD - Update for project specific extensions
