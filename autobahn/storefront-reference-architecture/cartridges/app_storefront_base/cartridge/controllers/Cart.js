...
server.post('AddProduct', function (req, res, next) {
    ...
    if (currentBasket) {
        Transaction.wrap(function () {
            if (!req.form.pidsObj) {
                quantity = req.form.quantity && !isNaN(parseInt(req.form.quantity, 10)) ? parseInt(req.form.quantity, 10) : 1;
                ...
                pidsObj.forEach(function (PIDObj) {
                    quantity = PIDObj.qty && !isNaN(parseInt(PIDObj.qty, 10)) ? parseInt(PIDObj.qty, 10) : 1;
...
server.get('RemoveProductLineItem', function (req, res, next) {
    ...
    Transaction.wrap(function () {
        if (req.querystring.pid && req.querystring.uuid) {
            ...
            var mainProdItem;
            var bonusItemUUIDs = new Set();
            for (var b = 0; b < bonusProductLineItems.length; b++) {
                bonusItemUUIDs.add(bonusProductLineItems[b].UUID);
            }
            for (var i = 0; i < productLineItems.length; i++) {
                var item = productLineItems[i];
                if ((item.UUID === req.querystring.uuid)) {
                    if (bonusProductLineItems && bonusProductLineItems.length > 0) {
                        for (var j = 0; j < item.lineItemCtnr.bonusLineItems.length; j++) {
                            if (bonusItemUUIDs.has(item.lineItemCtnr.bonusLineItems[j].UUID)) {
                            bonusProductsUUIDs.push(item.lineItemCtnr.bonusLineItems[j].UUID);
                            }
                        }
                    }
...
server.post('EditProductLineItem', function (req, res, next) {
    ...
    var productId = req.form.pid;
    // var selectedOptionValueId = req.form.selectedOptionValueId; AUTOBAHN MOD, use req.form.selectedOptionValueIds below
            ...
            Transaction.wrap(function () {
                ...
                if (optionModel && optionModel.options && optionModel.options.length) {
                    ...
                    while (productOptions.hasNext()) {
                        ...
                        for (var i = 0; i < selectedOptionValueIds.length; i++) {
