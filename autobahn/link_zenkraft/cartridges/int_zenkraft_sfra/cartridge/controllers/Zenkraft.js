...
server.post('GetShippingLabel', server.middleware.https, function (req, res, next) {
        ...
        if (syncReturnWithZenkraft) {
            ...
            Zenkraft.getSyncReturnRequest(order, returnID, returnsRequireApproval);
        }
        var returnObject = returnHelper.getReturnCase(returnID);
...
* @return {Object} Object representing the shipping label
*/
server.post('SendShippingLabel', server.middleware.https, function sendShippingLabelZenkraft(req, res, next) {
    var OrderMgr = require('dw/order/OrderMgr');
    var Zenkraft = require('~/cartridge/scripts/zenkraft');
