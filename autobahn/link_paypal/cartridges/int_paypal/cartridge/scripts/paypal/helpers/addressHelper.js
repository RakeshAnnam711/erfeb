...
addressHelper.updateOrderBillingAddress = function (basket, billingAddress) {
    ...
    Transaction.wrap(function () {
        ...
        if (empty(billing.phone) && phone && phone.phone_number) {
...
addressHelper.updateOrderShippingAddress = function (basket, shippingInfo, billingAddress) {
    var shipping = basket.getDefaultShipment().getShippingAddress();
    ...
    if (!shipping) {
        ...
        Transaction.wrap(function () {
            shipping = basket.getDefaultShipment().createShippingAddress();
            ...
            shipping.setPhone(billingAddress.phone && billingAddress.phone.phone_number ? billingAddress.phone.phone_number.national_number || '' : '');
...
addressHelper.updateBABillingAddress = function (basket, billingAddress) {
    const { first_name, last_name, billing_address, phone, email } = billingAddress;
...
* @param  {Object} billingAddress user's billing address
*/
addressHelper.updateBAShippingAddress = function (basket, shippingAddress, billingAddress) {
    var shipping = basket.getDefaultShipment().getShippingAddress();
    if (!shipping) {
        const { country_code, city, line1, line2, postal_code, state, recipient_name } = shippingAddress;
