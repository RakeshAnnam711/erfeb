...
function authorize(order, paymentInstrument) {
    const { isCapture, createTransaction, createOrder } = require('./paypalApi');
    ...
    Transaction.wrap(function () {
        ...
        paymentInstrument.getPaymentTransaction().setType(isCapture ? dw.order.PaymentTransaction.TYPE_CAPTURE : dw.order.PaymentTransaction.TYPE_AUTH);
    ...
    //Autobahn Customization, send back authResponse data for fraud providers
    return { authorized: true, authResponse: response };
}
