...
server.append('Show', function (req, res, next) {
    ...
    if (customer.authenticated && billingAgreementEnabled) {
        ...
        let savedPaypalBillingAgreements = billingAgreementModel.getBillingAgreements();

        if (savedPaypalBillingAgreements) {
            filterBillingAgreements(savedPaypalBillingAgreements, billingAgreementModel);
            savedPaypalBillingAgreements = billingAgreementModel.getBillingAgreements();
        }
        isUserHasSavedBA = !empty(savedPaypalBillingAgreements);
    ...
    //Redirect to payment page if missing phone number
    var paypalUrls = getUrls();
    if (basket && basket.billingAddress && !basket.billingAddress.phone) {
        paypalUrls.placeOrderStage = paypalUrls.paymentStage;
    }
    ...
    res.setViewData({
        paypal: {
            ...
            paypalUrls: JSON.stringify(paypalUrls)
        }
    });
    next();
});
