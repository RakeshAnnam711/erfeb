...
server.post('CreateBillingAgreement',
    ...
    function (req, res, next) {
        const { currentBasket } = require('dw/order/BasketMgr');
        let isAccountFlow = req.querystring.isAccountFlow === 'true';
        let paypalData;
        try {
            paypalData = req.body && JSON.parse(req.body);
        } catch (error) {
...
server.post('SaveBillingAgreement', server.middleware.https, function (req, res, next) {
    const BillingAgreementModel = require('../models/billingAgreement');
    const billingAgreementModel = new BillingAgreementModel();

    var baData = null;
    try {
        baData = req.body && JSON.parse(req.body);
    } catch (error) {
        createErrorLog(error);
        res.setStatusCode(500);
        res.print(createErrorMsg());
    ...
    if (baData) {
        let savedBA = billingAgreementModel.getBillingAgreements();
        let isAccountAlreadyExist = billingAgreementModel.isAccountAlreadyExist(baData.email);
...
server.post('FinishLpmOrder', server.middleware.https, function (req, res, next) {
    ...
    res.json({
        redirectUrl: URLUtils.https('Order-Confirm').toString(),
