...
BillingAgreementModel.prototype.removeBillingAgreement = function (ba) {
    ...
    var removedBAindex = this.billingAgreements
        .map(function (billingAgreement) { return billingAgreement.baID; })
        .indexOf(ba.baID);
...
/**
 * Check if billing agreement from customers profile is active
 * @param  {string} billingAgreementID Billing agreement id to check
 * @returns {boolean} true or false
 */
BillingAgreementModel.prototype.checkBillingAgreementStatus = function (billingAgreementID) {
    var baID = JSON.stringify({ baID: billingAgreementID });
    var paymentInstrumentMock = {
        custom: {
            PP_API_ActiveBillingAgreement: baID
        }
    };
    const { active } = getBADetails(paymentInstrumentMock);
