...
// AUTOBAHN MODIFICATION for calling pre-auth fraud check
...
function paymentFromComponent(req, res, next) {
  ...
  Logger.warn('autobahn_client_core\\paymentFromComponent: create order completed for the customer {0} :', currentBasket.customerEmail);
  if (!order) {
      Logger.warn('autobahn_client_core\\paymentFromComponent: create order failed for the customer {0} :', currentBasket.customerEmail);
      res.redirect(URLUtils.url('Checkout-Begin', 'stage', 'payment', 'paymentError', Resource.msg('error.technical', 'checkout', null)));
      return next();
  }

  // Call pre-auth fraud check
  ...
  if (preAuthFraudCheckResult.error) {
    ...
    res.json(preAuthFraudCheckResult);  // likely unnecessary/ineffective
    res.redirect(URLUtils.url('Checkout-Begin', 'stage', 'payment', 'paymentError', Resource.msg('error.technical', 'checkout', null)));
