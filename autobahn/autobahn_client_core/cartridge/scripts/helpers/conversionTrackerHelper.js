...
var TPA_COOKIE_NAME = 'Conversion';
var PAP_COOKIE_NAME = 'Conversion';
...
var conversionTrackerHelper = {
    parseCookieValueToObject: function (tpaCookieValue) {
        return JSON.parse(tpaCookieValue);
    },
    setConversionCookie: function (params) {
        var cookieValue = params || null;
        if (cookieValue) {
            setCookie(TPA_COOKIE_NAME, JSON.stringify(cookieValue), 7776000);  // 3600 * 24 * 90 =  7776000 seconds
            return cookieValue;
        }
    },
    getConversionCookie: function (COOKIE_NAME) {
        var tpaCookieValue = getCookieValue(COOKIE_NAME);
        if (!tpaCookieValue) {
            return null;
        }
        return conversionTrackerHelper.parseCookieValueToObject(tpaCookieValue);
    },
    deleteConversionCookie: function(COOKIE_NAME) {
        setCookie(COOKIE_NAME, '', -1);
    }
};

module.exports = conversionTrackerHelper;
