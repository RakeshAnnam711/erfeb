...
server.append('Confirm', function (req, res, next) {
    ...
    viewData.isLiveSellingOrder = !!(order && order.custom.isLiveSellingOrder);

    if (order && viewData.order && viewData.order.shipping) {
        var liveSellingHelpers = require('*/cartridge/scripts/helpers/liveSellingHelpers');

        viewData.order.shipping.forEach(function (shippingModel) {
            if (shippingModel.productLineItems) {
                liveSellingHelpers.markLiveSellingLineItems(order, shippingModel.productLineItems.items);
            }
        });
    }
