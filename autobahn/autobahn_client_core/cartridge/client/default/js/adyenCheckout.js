...
function displayMessage(data) {
  ...
  $('.add-to-wishlist-messages').append(
      $('<div>', {
          class: 'alert text-center add-to-wishlist-alert ' + status,
          text: data.msg // Safely sets the text content, escaping any HTML
      })
  );
...
$('button[value="submit-payment"]').on('click', function () {
  ...
  if (document.querySelector('#selectedPaymentOption').value === 'AdyenComponent' || document.querySelector('#selectedPaymentOption').value === 'CREDIT_CARD') {
    ...
    if(showValidation() === false){
      displayMessage({
          msg: 'Please enter payment details',
          type: 'warning'
      });
      scrollAnimate($('.payment-options-block'));
    }
