...
function callGetShippingMethods(req, res, next) {
  ...
  try {
    ...
    if (Site.getCurrent().getCustomPreferenceValue('enableZenkraftEstimatedDeliveryDates')) {
        ...
        zenkraftShippingMethods = zenkraftHelper.handleShippingMethods(zenkraftData);
    }
    var zenkraftShippingMethodsList = zenkraftShippingMethods.order.shipping[0].applicableShippingMethods;
    var zenkraftShippingMethodIds = new Set(zenkraftShippingMethodsList.map(item => item.ID));
    var zenkraftShippingMethodMap = zenkraftShippingMethodsList.reduce((acc, item) => {
        acc[item.ID] = item; // use id as key
        return acc;
    }, {});
    var newShippingMethodsModels = currentShippingMethodsModels.filter(item => zenkraftShippingMethodMap.hasOwnProperty(item.ID));
    newShippingMethodsModels.forEach(item => {
        var amount = 0.0;
        try {
            var zenkraftSM = zenkraftShippingMethodMap[item.ID];
            var cost = zenkraftSM.shippingCost == 'N/A' ? '0.0' : zenkraftSM.shippingCost.slice(1);
    ...
    currentShippingMethodsModels = newShippingMethodsModels;
