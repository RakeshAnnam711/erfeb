...
function createFileName(fileNamePrefix, fileExtension) {
    ...
    if (!fileExtension) {
        fileExtension = GoogleMerchantConstants.FILE_EXTENSTION.CSV; // eslint-disable-line no-param-reassign
    }
...
function getFileExtension(exportFormat) {
    if (exportFormat && exportFormat.toLowerCase() === GoogleMerchantConstants.FILE_EXTENSTION.XML) {
        return GoogleMerchantConstants.FILE_EXTENSTION.XML;
    }
    return GoogleMerchantConstants.FILE_EXTENSTION.CSV;
}
