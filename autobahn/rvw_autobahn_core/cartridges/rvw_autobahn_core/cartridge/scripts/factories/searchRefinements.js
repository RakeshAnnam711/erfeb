...
function createCategorySearchRefinement(productSearch, refinementDefinition, refinementValues, Model) {
    ...
    var findRefinementValue = function (category) {
        var defaultRefinementValue = {
            description: null,
            displayValue: null,
            hitCount: null,
            ID: null,
        ...
        return collections.find(refinementValues, function (refinementValue) {
            return !empty(category) && refinementValue.value == category.ID;
        }) || defaultRefinementValue;
    };

    if (currentCategory.root || currentCategory.parent.root) {
        topCategory = new Model(productSearch, refinementDefinition, findRefinementValue(currentCategory), currentCategory, true);
        insertPoint = topCategory.subCategories;
    } else {
        topCategory = new Model(productSearch, refinementDefinition, findRefinementValue(currentCategory.parent), currentCategory.parent);
        childCategory = new Model(productSearch, refinementDefinition, findRefinementValue(currentCategory), currentCategory, true);
    ...
    collections.forEach(currentCategory.subCategories, function (category) {
        if (category.online) {
            insertPoint.push(new Model(productSearch, refinementDefinition, findRefinementValue(category), category, false));
        }
    });
    return [topCategory];
}
