...
function validateOrder(basket, validateTax) {
    ...
    if (!basket) {
        ...
        if (productExistence.error || !productExistence.hasInventory) {
            ...
            result.errorMessage = Resource.msg('error.card.invalid.shipments', 'cart', null);
        } else if (basket.giftCertificateLineItems.length && basket.giftCertificatePaymentInstruments.length) {
            result.error = true;
            result.message = Resource.msg('giftcertificate.checkout.apply.noteligible', 'checkout', null);
        } else if (empty(basket.customerEmail)) {
            result.error = true;
            result.cartError = true;
            result.redirectUrl = dw.web.URLUtils.https('Checkout-Begin', 'stage', 'shipping').toString();
        }
    }
