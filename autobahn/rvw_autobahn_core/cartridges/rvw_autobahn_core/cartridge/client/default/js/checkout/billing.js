...
baseBilling.santitizeForm = function santitizeForm() {
    // Replace inherited serializer to guard checkout flows where Cleave is not initialized.
    $('body').off('checkout:serializeBilling');
        ...
        serializedArray.forEach(function (item) {
            if (item.name.indexOf('cardNumber') > -1) {
                if (cardNumberCleave && typeof cardNumberCleave.getRawValue === 'function') {
                    item.value = cardNumberCleave.getRawValue();
