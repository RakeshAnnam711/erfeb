        ...
        $form.data('defer', recaptcha.check(e, {
            ...
            success: function (data) {
                if (data?.success !== true) {
                    ...
                    // Push password reset event to window.dataLayer
                    const session = document.cookie.split('; ').find(c => c.startsWith('_bts='))?.split('=')[1];
                    window.dataLayer = window.dataLayer || [];
                ...
                window.dataLayer.push(resetPasswordEvent);
                console.log("Password reset event pushed to dataLayer");
                $form.spinner().stop();
                $form.removeClass('csrf-used');
