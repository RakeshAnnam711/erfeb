...
module.exports = function bonusOrderLineItem(product, apiProduct, options) {
    ...
    if (apiProduct.bundledProducts.length > 0) {
        productLineItemDecorators.bundledProductLineItems(product, options.lineItem, this);
    }
    productLineItemDecorators.discountBonusLineItems(product, product.bonusProductLineItemUUID);
