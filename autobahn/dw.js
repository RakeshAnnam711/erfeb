...
const hasOnlyEmptyValues = value => Array.isArray(value) && value.every(entry => String(entry || '').trim() === '');

if (hasOnlyEmptyValues(dwjson.cartridge)) {
    delete dwjson.cartridge;
}

if (hasOnlyEmptyValues(dwjson.dataFor)) {
    delete dwjson.dataFor;
}
