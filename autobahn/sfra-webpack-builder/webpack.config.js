 ...
 */
class WebpackBundle {
  static isPaypalCartridge(cartridge) {
    return cartridge.indexOf("/cartridges/int_paypal") !== -1;
  }

  static addPaypalPrefixedEntry(files, sourceRelativePath, sourceName, source) {
    if (!this.isPaypalCartridge(source) && !this.isPaypalCartridge(sourceRelativePath)) {
      return;
    }

    if (sourceName.indexOf("int_paypal_") === 0) {
      return;
    }

    const prefixedOutputFile = path
      .join(sourceRelativePath, `int_paypal_${sourceName}`)
      .split(path.sep)
      .join(path.posix.sep);
    files[prefixedOutputFile] = source;
  }
      ...
      glob
        ...
        .forEach((source) => {
          ...
          const outputFile = path
            ...
            .join(path.posix.sep);
          files[outputFile] = source;
          if (this.isPaypalCartridge(cartridge)) {
            this.addPaypalPrefixedEntry(files, sourceRelativePath, sourceName, source);
          }
        });
  ...
  static getPlugins(cartridge, fileType, env) {
    ...
    if (fileType === "clean") {
      ...
      if (env.noSCSS !== true) {
        const cssCleanPatterns = this.isPaypalCartridge(cartridge)
          ? ["*/css/*", "!*/css/paypal.css", "!*/css/global.css"]
          : ["*/css"];

        plugins.push(
            new CleanWebpackPlugin({
            cleanOnceBeforeBuildPatterns: cssCleanPatterns,
            verbose: false,
            })
        );
      }
